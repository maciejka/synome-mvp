...
public class ChangesetProcessor {
    ...
    public ChangesetResponse process(Changeset changeset) {
        ...
        try {
            ...
            for (DerivationTracker.Derivation d : tracker.getDerivations()) {
                ...
                derivedFacts.add(new DerivedFactSummary(
            ...
            LOG.infof("Changeset %s applied: seq=%d, rulesFired=%d, derived=%d, retracted=%d, duration=%dms",
                    changeset.id(), seqNum, rulesFired,
                    effects.derivedFactsCreated, effects.derivedFactsRetracted, durationMs);
    ...
    private void applyEntry(ChangesetEntry entry, KieSession session,
                            FactRegistry registry, EffectsSummary effects) {
        switch (entry.action()) {
    ...
    private void applyUpsert(ChangesetEntry entry, KieSession session,
                             FactRegistry registry, EffectsSummary effects) {
        ...
        if (existing != null) {
            ...
            registry.put(entry.factKey(), new FactRegistry.FactEntry(newHandle, entry.factType(), entry.data()));
            effects.factsUpdated++;
        } else {
            // Insert new fact
            FactHandle handle = session.insert(fact);
            registry.put(entry.factKey(), new FactRegistry.FactEntry(handle, entry.factType(), entry.data()));
            effects.factsInserted++;
        }
    }

    private void applyDelete(ChangesetEntry entry, KieSession session,
                             FactRegistry registry, EffectsSummary effects) {
